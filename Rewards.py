...
class Rewards:
    ...
    def getRewards(self, full_cands):
        ...
        for i in range(len(self.facts)):
            ...
            for name in self.reward_names:
                if name == 'recall':
                    ...
                    r.append(self.fact_order(
                        partial_cand, ref_order, facts, fields))
                elif name == 'length':
        ...
        if 'extractor' in self.reward_names:
            ...
            for i in range(0, len(all_refs), small_batch_size):
                raw_scores = self.extract_score(
                    all_refs[i: i+small_batch_size], self.extractor_gold_answer[i:i+small_batch_size])
                modified_scores = torch.from_numpy(
                    raw_scores) * self.extractor_field_mask_tensor[i:i+small_batch_size]
                temp_mask_sum = self.extractor_field_mask_tensor[i:i+small_batch_size].sum(
                    1).view(-1)

                modified_scores = modified_scores.sum(
                    1).view(-1) / temp_mask_sum
    ...
    def extract_score(self, all_ref, gold_answers, score_return_type="AM"):
        ...
        for et_id in range(len(extract_types)):
            ...
            gold_answer = [gold_answers[i][extract_type]
    ...
    def perplexity(self, partial_cand):
        ...
        encodings = self.gpt2_tokenizer(
            partial_cand, return_tensors='pt', padding=True, truncation=True, max_length=max_length)
        ...
        for i in range(0, encodings.input_ids.size(1), stride):
            ...
            with torch.no_grad():
                ...
                output = logits.gather(2, target_ids[:, 1:, :]).squeeze(2)[
                    :, -length:]
                mask = mask[:, 1:][:, -length:]
